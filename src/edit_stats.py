...
def apply_profession_change(data: bytes, player: dict, new_num: int,
    ...
    Only swaps the StartingPerk enum string in-place.  The NE2 skill-tree
    entries are left untouched because the game's Index→Profession mapping
    is dynamic and per-player; blindly locking entries puts the profession
    system into an inconsistent state (prereqs shown but no unlock button).
    """
    _ensure_backup(save_file, backup_created)
